...
cd "$repo_root"
if [[ $# != 2 || ! " layout button viewport settings collections panes navigation records " =~ " $1 " ]]; then
  echo "Usage: scripts/capture-specimen.sh {layout|button|viewport|settings|collections|panes|navigation|records} OUTPUT_DIRECTORY" >&2
  exit 2
fi
