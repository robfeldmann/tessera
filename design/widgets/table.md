...
throughout, per [Slice 7](../../docs/Spec.md#slice-7-widgets--textinput-list-scrollview):
the app owns rows, selection, and sort order through bindings; Table owns only a scroll
offset in `NodeState`. Table renders chrome (header, rule, selection, scrollbar) but never
invents data, selection, or ordering.
...
1. r0       Header row -- Text per column, token `table.header`; pinned, never scrolls
...
5. r2-r7    Data rows -- one row per visible element; 6 of 12 visible here
6. r6 c14   Truncation -- Text `tail` policy, `truncation.mark`, grapheme-safe
7. c45      Scrollbar -- track `scrollbar.track`, thumb `scrollbar.thumb`,
            thumb = max(1, visible/total x track) = 3 of 6
...
| --------------- | ----------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| rows            | Binding     | `Data` (RandomAccessCollection) | app-owned; Table re-resolves layout on every update                                                                 |
...
| scroll offset   | NodeState   | `Int` (first visible row)       | clamps to `0...max(0, count - visible)` on every update; keeps selection visible after selection-moving keys        |
| column widths   | derived     | `[Int]`                         | recomputed per layout pass from column policy; never stored                                                         |
| header styling  | Environment | token `table.header`            | from [tokens.md](../tokens.md)                                                                                      |
...
| wheel-down   | data row    | offset below max       | increments NodeState scroll offset                | yes      |
| wheel-up     | data row    | offset above 0         | decrements NodeState scroll offset                | yes      |
| drag         | scrollbar   | always                 | sets offset proportionally to drag position       | yes      |

Wheel scrolling moves the viewport without touching selection (selection may scroll out of
view; the bar and fill simply are not rendered until it returns).
...
| --------- | ------ | ------------------------------------------------------------------------------------------------------------------------------- |
| nil x nil | 46x14  | ideal: resolved column widths + gutter + scrollbar by content width; height = header + rule + all 12 rows (no footer: app slot) |
...
- `table.header` -- header text style (proposed addition to [tokens.md](../tokens.md))
...
- Text truncation policies (`tail`, grapheme-safe) -- needs `primitives/text.md`. Missing;
  blocks `ready`.
- ScrollIndicator -- shared with ScrollView and List; needs
  `primitives/scroll-indicator.md`. Missing; blocks `ready`.
...
- `vanished selection id renders no selection and does not mutate the binding` (state
  model: selection)
- `scroll offset clamps when app shrinks data` (state model: scroll offset)
