...
### Key provenance and phases

Every ``Key`` carries ``Key/source`` as well as ``Key/kind``. Provenance is part of the
value's `Equatable` identity; consumers must not infer a future release from a lone
``KeyEventKind/press`` event.

| Source | Report | Meaning |
| --- | --- | --- |
| ``KeyEventSource/legacy`` | Legacy byte-oriented report | Press-only input with no release provenance. |
| ``KeyEventSource/kittyPressOnly`` | CSI-u with no event-kind field | Kitty press-only input; treat it like an immediate press. |
| ``KeyEventSource/kitty`` | CSI-u with an event-kind field | `:1` press, `:2` repeat, or `:3` release. |

This distinction lets a responder provide immediate activation for legacy and bare CSI-u
reports while pairing only explicit Kitty phases. ``KeyEventSource/kitty`` is still the
source for a phased event even when its kind is ``KeyEventKind/repeat``.

### Normalize pointer phases

``PointerEvent/init(mouse:)`` maps a parsed ``MouseEvent`` to the responder-facing
``PointerPhase`` vocabulary: press becomes ``PointerPhase/down``, release becomes
``PointerPhase/up``, drag or motion becomes ``PointerPhase/move``, and cancellation is
generated by the graph when an interaction can no longer complete. Scroll reports are not
pointer phases and return `nil` from this initializer. A `move` describes motion only;
it does not imply hover state.
