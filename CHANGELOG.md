...
### Added
- Added read-only automation identifiers and exact selector lookup, versioned semantic capture metadata, and failure bundles that retain completed frames without sending ambiguous input.
...
- Corrected Button pointer capture cancellation, disabled ancestor routing, source-aware phased key handling, baseline focus reporting, and visible compact/plain pressed feedback with regression coverage.
...
- Added the provisional Phase 4 viewport and interaction contracts: clamped `ScrollView` offset, keyboard/wheel input, focus reveal, scroll indicators, hover enter/exit, normalized pointer phases, and nested-boundary bubbling.
- Added controlled single-line `TextField` editing with grapheme-safe local cursor/selection and click-to-caret behavior; committed input and paste remain distinct from deferred IME pre-edit and secure-entry masking.
- Added controlled `Toggle`, `Stepper`, and `Picker` values, keyed `List`/`Section`/`Table` collection semantics, row-major `Grid`, negotiated `SplitView`, and regular/compact `NavigationSplitView` roles.
